...
export async function updateTable(id: string, input: TableFormInput): Promise<WeddingTable> {
  ...
  if (error) throw new Error(`No se pudo actualizar la mesa: ${error.message}`);
  return mapTable(data);
}
...
export async function deleteTable(id: string): Promise<void> {
  ...
  if (countError) throw new Error(`No se pudo comprobar la mesa: ${countError.message}`);
  if ((count ?? 0) > 0) throw new Error("No se puede borrar una mesa con pruebas enviadas.");
...
export async function getTableUsage(weddingId: string): Promise<TableUsage> {
  ...
  if (error) throw new Error(`No se pudo calcular el uso de mesas: ${error.message}`);
